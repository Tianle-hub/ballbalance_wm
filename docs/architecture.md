...
The world model encodes normalized observations and runs posterior inference over `obs[0:T+1]` with actions `action[0:T]`. Dreamer behavior learning then flattens posterior states across batch and time and starts imagined rollouts from those states.
