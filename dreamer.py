...
class Dreamer:
    ...
    def actor_loss(self, start_nonterms=None):
        ...
        with FreezeParameters(self.world_model_modules + self.value_modules):
            ...
            if self.args.use_disc_model:
                ...
                discounts = self.args.discount * imag_disc_dist.mean
            else:
                discounts = self.args.discount * torch.ones_like(imag_rews)

        if start_nonterms is not None:
            first_nonterms = torch.reshape(start_nonterms, (-1, start_nonterms.shape[-1]))
            discounts = torch.cat([self.args.discount * first_nonterms.unsqueeze(0), discounts[1:]], 0)

        self.returns = compute_return(
            imag_rews[:-1], imag_vals[:-1], discounts[:-1],
            self.args.td_lambda, imag_vals[-1])

        weights = torch.cumprod(torch.cat([torch.ones_like(discounts[:1]), discounts[:-1]], 0), 0)
        self.discounts = weights[:-1].detach()

        policy = self.actor.get_dist(self.imag_feat[:-2].detach())
        actor_grad = getattr(self.args, 'actor_grad', 'dynamics')
        if actor_grad == 'dynamics':
            objective = self.returns[1:]
        elif actor_grad == 'reinforce':
            baseline = imag_vals[:-2]
            advantage = (self.returns[1:] - baseline).detach()
            action = imag_actions[1:-1].detach()
            objective = policy.log_prob(action).unsqueeze(-1) * advantage
        elif actor_grad == 'both':
            baseline = imag_vals[:-2]
            advantage = (self.returns[1:] - baseline).detach()
            reinforce_objective = policy.log_prob(imag_actions[1:-1]).unsqueeze(-1) * advantage
            mix = getattr(self.args, 'actor_grad_mix', 0.1)
            objective = mix * self.returns[1:] + (1 - mix) * reinforce_objective
        else:
            raise NotImplementedError(actor_grad)

        entropy = policy.entropy().unsqueeze(-1)
        objective = objective + getattr(self.args, 'actor_ent', 0.0) * entropy
        actor_loss = -torch.mean(weights[:-2].detach() * objective)
        return actor_loss
    ...
    def train_one_batch(self):
        ...
        actor_loss = self.actor_loss(nonterms[:-1])
...
def main():
    ...
    parser.add_argument('--exp-name', '--exp', dest='exp_name', type=str, default='lr1e-3', help='name of experiment for logging')
    ...
    parser.add_argument('--discrete-classes', type=int, default=32, help='Number of classes per categorical latent for Dreamerv2')
    ...
    parser.add_argument('--actor-grad', type=str, default='dynamics', choices=['dynamics', 'reinforce', 'both'], help='Actor gradient estimator used in imagination')
    parser.add_argument('--actor-grad-mix', type=float, default=0.1, help='Dynamics objective fraction when actor-grad is both')
    parser.add_argument('--actor-ent', type=float, default=1e-4, help='Actor entropy regularization scale')
