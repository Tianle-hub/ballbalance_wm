...
def compute_return(rewards, values, discounts, td_lam, last_value):
    """Compute TD(lambda) returns for a sequence.

    This implements the reverse recursion:
        R_t = r_t + d_t * ((1 - lambda) * V_{t+1} + lambda * R_{t+1})
    where ``last_value`` is the bootstrap value used as R_{H}.

    R_t is the lambda-return target
    """

    # Build V_{t+1} for every timestep. For the final reward, V_{H} is
    # provided separately as ``last_value``.
    next_values = torch.cat([values[1:], last_value.unsqueeze(0)], 0)

    # This is r_t + d_t * (1 - lambda) * V_{t+1}; the lambda-weighted
    # recursive return d_t * lambda * R_{t+1} is added inside the loop.
    reward_with_bootstrap = rewards + discounts * next_values * (1 - td_lam)
    ...
    return torch.flip(torch.stack(returns), [0])
